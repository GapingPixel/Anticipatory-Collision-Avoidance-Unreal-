...
#include "Camera/ADogCameraMode_ThirdPerson.h"
#include "Camera/ADogCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/ADogPenetrationAvoidanceFeeler.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ADogCameraMode_ThirdPerson)

namespace ADogCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UADogCameraMode_ThirdPerson::UADogCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FADogPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FADogPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FADogPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FADogPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FADogPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FADogPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FADogPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UADogCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UADogCameraMode_ThirdPerson::OnActivation()
{
	Super::OnActivation();

	// Initialize this so its not 0,0,0
	PreviousDesiredLoc = GetADogCameraComponent()->GetComponentLocation();
}

void UADogCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UADogCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UADogCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IADogCameraAssistInterface* TargetControllerAssist = Cast<IADogCameraAssistInterface>(TargetController);

	IADogCameraAssistInterface* TargetActorAssist = Cast<IADogCameraAssistInterface>(TargetActor);
	...
	IADogCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IADogCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IADogCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IADogCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UADogCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IADogCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FADogPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			{
				DrawDebugSphere(World, SafeLoc, SphereShape.Sphere.Radius, 8, FColor::Red,false, -1);
				DrawDebugSphere(World, bHit ? Hit.Location : RayTarget, SphereShape.Sphere.Radius, 8, FColor::Blue, false, -1);
				DrawDebugLine(World, SafeLoc, bHit ? Hit.Location : RayTarget, FColor::Green);
			}
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(ADogCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
	...
	{
		//DrawDebugSphere(World, CameraLoc, 15, 16, FColor::Purple,false, 20);
		CameraLoc = SafeLoc + (CameraLoc - SafeLoc) * DistBlockedPct;
		//DrawDebugSphere(World, CameraLoc, 15, 18, FColor::Yellow,false, 20);
	}
}

void UADogCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UADogCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
