...
#include "Audio/ADogAudioSettings.h"
#include "Audio/ADogAudioMixEffectsSubsystem.h"
#include "EnhancedActionKeyMapping.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(ADogSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("AlphaDog.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("AlphaDog.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("AlphaDog.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("ADog.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("ADog.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("ADog.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("ADog.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("ADog.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("ADog.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("ADog.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FADogScalabilitySnapshot::FADogScalabilitySnapshot()
{
...
namespace ADogSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FADogScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UADogSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UADogSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UADogSettingsLocal::UADogSettingsLocal()
{
...
void UADogSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UADogSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UADogPlatformSpecificRenderingSettings* PlatformSettings = UADogPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EADogFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	ADogSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UADogSettingsLocal::ResetToCurrentSettings()
{
...
void UADogSettingsLocal::BeginDestroy()
{
...
UADogSettingsLocal* UADogSettingsLocal::Get()
{
	return GEngine ? CastChecked<UADogSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UADogSettingsLocal::ConfirmVideoMode()
{
...
float UADogSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UADogPlatformSpecificRenderingSettings* PlatformSettings = UADogPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EADogFramePacingMode::DesktopStyle)
	{
...
int32 UADogSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return ADogSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UADogSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FADogScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UADogSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UADogSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UADogSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UADogSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UADogSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UADogSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EADogStatDisplayMode UADogSettingsLocal::GetPerfStatDisplayState(EADogDisplayablePerformanceStat Stat) const
{
	if (const EADogStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EADogStatDisplayMode::Hidden;
	}
}

void UADogSettingsLocal::SetPerfStatDisplayState(EADogDisplayablePerformanceStat Stat, EADogStatDisplayMode DisplayMode)
{
	if (DisplayMode == EADogStatDisplayMode::Hidden)
	{
...
float UADogSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UADogSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UADogSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UADogSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UADogSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UADogSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UADogSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UADogSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UADogSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UADogSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UADogSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UADogSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UADogSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UADogSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UADogSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UADogSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return ADogSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UADogSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return ADogSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UADogSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UADogSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UADogPlatformSpecificRenderingSettings* PlatformSettings = UADogPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EADogFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return ADogSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UADogSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UADogPlatformSpecificRenderingSettings* PlatformSettings = UADogPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EADogFramePacingMode::MobileStyle)
	{
...
void UADogSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UADogSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UADogPlatformSpecificRenderingSettings* PlatformSettings = UADogPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EADogFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = ADogSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UADogSettingsLocal::ClampMobileQuality()
{
	const UADogPlatformSpecificRenderingSettings* PlatformSettings = UADogPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EADogFramePacingMode::MobileStyle)
	{
...
void UADogSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = ADogSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UADogSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = ADogSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = ADogSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UADogSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UADogSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UADogSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UADogSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UADogSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = ADogSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UADogSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UADogSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UADogAudioMixEffectsSubsystem* ADogAudioMixEffectsSubsystem = World->GetSubsystem<UADogAudioMixEffectsSubsystem>())
			{
				ADogAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UADogSettingsLocal::CanRunAutoBenchmark() const
{
	const UADogPlatformSpecificRenderingSettings* PlatformSettings = UADogPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UADogSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UADogSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UADogSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UADogSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UADogSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UADogSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UADogSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UADogSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UADogSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UADogSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UADogSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UADogSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UADogSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UADogSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UADogSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UADogSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UADogSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UADogSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UADogSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EADogFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = ADogSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UADogSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UADogSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UADogSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UADogAudioSettings* ADogAudioSettings = GetDefault<UADogAudioSettings>())
			{
				...
				if (UObject* ObjPath = ADogAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from ADog Audio Settings."));
					}
				}

				if (UObject* ObjPath = ADogAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from ADog Audio Settings."));
					}
				}

				if (UObject* ObjPath = ADogAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from ADog Audio Settings."));
					}
				}

				if (UObject* ObjPath = ADogAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from ADog Audio Settings."));
					}
				}

				if (UObject* ObjPath = ADogAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from ADog Audio Settings."));
					}
				}

				if (UObject* ObjPath = ADogAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from ADog Audio Settings."));
					}
				}
			}
		}
...
void UADogSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UADogSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UADogPlatformSpecificRenderingSettings* PlatformSettings = UADogPlatformSpecificRenderingSettings::Get();
	const TArray<FADogQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FADogQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UADogPlatformEmulationSettings* Settings = GetDefault<UADogPlatformEmulationSettings>();
	...
	{
	case EADogFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EADogFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EADogFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UADogSettingsLocal::UpdateConsoleFramePacing()
{
...
void UADogSettingsLocal::UpdateDesktopFramePacing()
{
...
void UADogSettingsLocal::UpdateMobileFramePacing()
{
...
void UADogSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
