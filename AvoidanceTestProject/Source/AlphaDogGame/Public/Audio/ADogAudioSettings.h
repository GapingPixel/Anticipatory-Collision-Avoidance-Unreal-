...
#include "ADogAudioSettings.generated.h"
...
USTRUCT()
struct ALPHADOGGAME_API FADogSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "ADogAudioSettings"))
class ALPHADOGGAME_API UADogAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FADogSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FADogSubmixEffectChainMap> LDRAudioSubmixEffectChain;
