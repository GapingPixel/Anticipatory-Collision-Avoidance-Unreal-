...
#include "ADogInputModifiers.generated.h"

struct FInputActionValue;

class FProperty;
class UEnhancedPlayerInput;
class UADogAimSensitivityData;
class UObject;
...
class UADogSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the ADog Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "AlphaDog Settings Driven Dead Zone"))
class UADogInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EADogTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in ADog Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "AlphaDog Gamepad Sensitivity"))
class UADogInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	EADogTargetingType TargetingType = EADogTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const UADogAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the ADog Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "AlphaDog Aim Inversion Setting"))
class UADogInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
