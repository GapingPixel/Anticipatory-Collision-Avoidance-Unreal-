...
#include "ADogSettingsLocal.generated.h"
...
USTRUCT()
struct FADogScalabilitySnapshot
{
	GENERATED_BODY()

	FADogScalabilitySnapshot();
 ...
 */
UCLASS()
class UADogSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EADogStatDisplayMode GetPerfStatDisplayState(EADogDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EADogDisplayablePerformanceStat Stat, EADogStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UADogSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EADogDisplayablePerformanceStat, EADogStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FADogScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FADogScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UADogSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
