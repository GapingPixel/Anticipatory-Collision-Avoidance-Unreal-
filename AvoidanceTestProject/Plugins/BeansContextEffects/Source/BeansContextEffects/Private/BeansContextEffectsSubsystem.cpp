...
#include UE_INLINE_GENERATED_CPP_BY_NAME(BeansContextEffectsSubsystem)
...
void UBeansContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<UBeansContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (UBeansContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (UBeansContextEffectsLibrary* EffectLibrary : EffectsLibraries->BeansContextEffectsLibraries)
			{
...
bool UBeansContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const UBeansContextEffectsSettings* ProjectSettings = GetDefault<UBeansContextEffectsSettings>())
	{
...
void UBeansContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<UBeansContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	UBeansContextEffectsSet* EffectsLibrariesSet = NewObject<UBeansContextEffectsSet>(this);

	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<UBeansContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (UBeansContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->BeansContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void UBeansContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
