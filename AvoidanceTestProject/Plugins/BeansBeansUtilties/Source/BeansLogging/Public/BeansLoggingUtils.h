...
#include <cxxabi.h>
#include <execinfo.h>
static char *class_func(const char *c, const char *f)
{
	int status;
...
// UObject derived
#define BEANS_ULOG(logcategory, verbosity, fmt, ...) \
	UE_LOG(logcategory, verbosity, \
		TEXT("[%s] %s:%i [%s]->[%s] %s"), \
		*ClientServerContext(this), \
...
// Subsystem derived
#define BEANS_SLOG(logcategory, verbosity, fmt, ...) \
	UE_LOG(logcategory, verbosity, \
		TEXT("[%s] %s:%i %s"), \
		*ClientServerContext(this), \
		*FString(__FUNCTION__), __LINE__, \
...
static FString ClientServerContext(const UObject* WorldContextObject)
{
